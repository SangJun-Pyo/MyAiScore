...
제품명: MyAiScore / My AI Score. 현재 제품 UI는 영어 전용이며 대표 문구는 “Build with AI. Know your part.”다. 사용자 제출 원문과 과거 기록은 소급 번역하지 않고 내부 기획 문서의 언어도 그대로 유지한다.
...
사용자의 2026-09-14 지시에 따라 Astra가 구현·통합까지 맡고 서브에이전트에 독립 작업·검토를 위임한다. 모델/계정 미정 상태에서 웹·API·배포 설정까지 병행 구현하며 실제 모델 교정과 배포 실적은 분리한다. 상준님은 제품과 결과 수용 판단을 담당한다.
