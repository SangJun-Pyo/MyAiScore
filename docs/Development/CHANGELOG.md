...
- 직접 확인/파이프라인, 설계 선행/사후 추적 등 각 차원의 독립 강도는 화면에서 상대 비중으로 정규화해 좌우 표시값이 항상 100%가 되도록 했다. 원본 강도와 유형 선택 규칙은 변경하지 않았다.
...
- 하단 16유형은 코드의 네 글자를 그대로 반영한 4분할 색 띠를 사용하며, 카드를 펼치면 유형 설명을 확인할 수 있다. 펼침 화살표와 키보드 포커스도 함께 제공한다.
- 기존 살구색은 일반 강조, 민트는 만점·미확인 신호 없음 상태라는 의미를 그대로 유지한다.
- 자세한 내용: [Phase-09 세션 로그 2026-09-20](Sessions/Phase-09-Anonymous-Repository-Reports.md#2026-09-20--sjti-4글자-코드에-골드바이올렛-방향-구분-추가)
- 검증: `npm run typecheck`, `npm test` 320/320, `npm run check:docs` 74파일·441개 링크, `npm run build`, 해석 가이드 집중 E2E 4/4, WCAG·키보드·320px E2E 6/6, 전체 브라우저 E2E 42/42를 통과했다.

## 2026-09-20 — 결과 화면 상태 기반 강조 색상

- "추천 개선 과제" 카드의 살구색 풀블록 배경을 다크 패널 + 좌측 강조 바로 바꿔 다른 카드들과 톤을 맞췄다.
- "미확인 신호" 카드는 실제로 놓친 신호가 없을 때만(`repo-gaps-clear`) 민트 색으로 강조되고, 문구도 더 명확하게 다듬었다.
- 4축 신호 카드(맥락/검증 기반/기록/자동화)도 같은 원칙으로, 개별 근거 항목은 만점일 때만 배지가, 축 전체가 25/25 만점일 때만 카드 전체가 민트로 강조된다.
- 실행 환경 제약으로 이 변경은 `npm test`/`typecheck`/`build`를 실행하지 못한 채 커밋됐다 — 자세한 내용은 [Phase 9 세션](Sessions/Phase-09-Anonymous-Repository-Reports.md#2026-09-20--결과-화면-상태-기반-강조-색상) 참고.

## 2026-09-19 — AI 활용 질문형 홈 제목

- 홈 H1을 `AI로 코딩하고 있는데, 제대로 활용하고 있을까요?`로 바꿔 사용자가 궁금해하는 목적을 더 직접적으로 표현했다.
- 공개 저장소 기반이라는 측정 범위와 개인 능력 인증이 아니라는 한계는 바로 아래 설명과 기존 안내에서 계속 밝힌다.

## 2026-09-19 — 결과 스타일명과 SJTI 설명 정리

- 6개 축 기반 결과 스타일명을 `스타터 빌더`, `균형 잡힌 빌더`, `맥락 설계자`, `검증 수호자`, `기록 전략가`, `자동화 지휘자`로 정리했다.
- SJTI 카드 설명을 “가장 가까운 유형”이라는 일반 문구 대신 각 유형의 특징 설명으로 바꾸고, `검증 루프 항해사` 설명은 테스트·직접 확인과 기록·추적 단서의 관계가 두 줄로 읽히도록 했다.
- “개인의 능력이나 성격 평가가 아니다”라는 한계 문구는 해석 가이드의 하단 단일 안내문으로 옮겨 반복 노출을 줄였다.

## 2026-09-19 — 홈 질문형 헤드라인

- 추상적인 `공개 저장소에서 AI 협업을 뒷받침하는 신호를 찾습니다.`를 사용자의 목적을 바로 묻는 `내 저장소에는 AI를 잘 활용한 흔적이 얼마나 남아 있을까요?`로 바꿨다.
- 개인 능력을 직접 인증한다고 단정하지 않고, 관찰 대상이 저장소에 남은 흔적이라는 경계는 제목 안에 유지했다.

## 2026-09-19 — 축 기반 스타일 복원과 SJTI 명칭

- 해석 가이드에서 `균형 잡힌 빌더` 같은 축 기반 결과 스타일 목록을 SJTI 네 글자 유형 설명 위에 항상 표시하도록 복원했다.
- 네 글자 코드 체계를 `SJTI — Sang-Jun Type Indicator`로 명명해, 결과 카드의 축 기반 스타일명과 별도의 저장소 신호 유형 라벨임을 분리했다.
- 저장 리포트가 선택된 상태에서는 축 기반 스타일 목록에서 현재 리포트 스타일을 다시 강조한다.

## 2026-09-19 — 레이더 카드 배치와 점수 규칙 기본 접힘

- 결과 카드 우측 영역의 순서를 협업 유형 요약 → 4축 레이더로 바꿔, 설명을 먼저 읽고 그래프를 확인하는 흐름으로 조정했다.
- Logic Core 장식을 좌측 패널 안에서 더 왼쪽으로 이동시키고, 상단 `저장소에서 확인된 협업 특성` 칩이 내용 폭만 차지하도록 되돌렸다.
- 레이더 축 라벨을 그래프 바깥 compass 라벨로 분리해 그래프 점·선과 겹치지 않게 했다.
- 해석 가이드의 점수 규칙 카드는 기본 접힘 상태로 변경했다.

## 2026-09-19 — 결과 카드 레이더 영역 분리

- 결과 상단 카드에서 Logic Core 장식을 좌측 정보 패널 내부로 제한해 우측 레이더와 배경 도형이 겹치지 않게 했다.
- 4축 레이더 SVG 안에 맥락·검증 기반·기록·자동화 축 라벨을 직접 표시하고, 우측 레이더/유형 요약 영역을 좌측 점수 정보와 별도 패널처럼 구분했다.
- 협업 유형의 `유형 신뢰도` 문구와 경계선 사유 목록은 화면에서 제거했다. 네 글자 코드는 보조 라벨로 남기되, 정확한 신뢰도 판정처럼 보이지 않게 한다.

## 2026-09-19 — 데모 매트릭스와 접근성 사전 점검

- 고정 SHA 코호트에서 다섯 생태계, 세 규모 구간, complete/partial coverage와 7~71점 범위를 대표하는 시연 저장소 5개를 고정했다.
- 시연 전 health, 성공·부분 수집·오류, PNG 공유 카드, 브라우저 저장 흐름을 확인하는 runbook을 추가했다.
- 공개 네 화면에 WCAG A/AA serious·critical 위반 검사와 건너뛰기 링크, 동작 감소, 320px 가로 넘침 회귀 검사를 추가했다.
- 자동 검사에서 근거 파일 펼침 버튼의 대비가 4.22:1인 문제를 발견해 공용 보조 글자색을 밝히고 AA 최소 4.5:1 이상으로 보정했다.
- Railway에서 complete·partial 대표 저장소를 다시 분석하고, production build의 데스크톱·모바일 전송량과 렌더링 기준점을 기록했다.
- 메인에 이미 포함된 `codex/uplink-loader-progress`, `codex/threeui-intro-hero` 원격 잔존 브랜치를 삭제했다.

## 2026-09-19 — 결과 카드 의미 분리와 4축 레이더

- 결과 상단 카드의 큰 제목을 D/R/H/P/S/T/F/E 협업 유형명이 아니라 맥락·검증·기록·자동화 점수 분포로 정해지는 저장소 신호 스타일명으로 되돌렸다. 네 글자 협업 유형 코드는 별도 보조 요약으로 분리해 능력·성격 유형처럼 보이지 않게 했다.
- 해석 가이드의 저장 리포트 요약 카드 우측에 4축 레이더 차트와 협업 유형 설명 문구를 추가해 넓은 화면의 빈 공간을 결과 데이터로 채웠다.
- 저장소 분석 결과 화면과 홈 하단 예시 리포트도 해석 가이드와 같은 단일 다크 카드 레이아웃을 사용하도록 통일했다.

## 2026-09-19 — 해석 가이드 유형 목록과 파비콘

- 해석 가이드에 16가지 협업 유형 코드와 이름을 모두 표시해, 선택된 리포트의 네 글자 코드가 어떤 이름 체계에서 나온 것인지 확인할 수 있게 했다.
- 점수 규칙 카드의 내용을 기본 펼침 상태의 접이식 패널로 바꿔, 긴 배점표를 필요할 때만 열어 볼 수 있게 했다.
- MyAiScore 브랜드마크 색상에 맞춘 SVG favicon을 연결했다.

## 2026-09-19 — 홈·해석 가이드 UI와 분석 로딩 안정화

- 다른 화면 폭에서 홈 H1이 예시 카드 영역을 침범하지 않도록 글자 크기 상한과 텍스트 폭을 낮추고, 한국어 단어 단위 줄바꿈을 허용했다.
- 해석 가이드(`/insights`)는 현재 main의 상세 리포트와 점수 해석 구조를 유지하되, 선택된 리포트의 상단 요약은 홈 예시 카드와 같은 단일 다크 카드 스타일로 표시한다.
- 해석 가이드의 신호별 점수 규칙 표를 다크 패널 톤에 맞춰 배경·테두리·축 합계 강조를 조정했다.
- 저장소 분석 API 응답이 UplinkLoader의 첫 100% 이후에 도착해도 다음 루프의 100%를 다시 기다리지 않고 결과로 전환한다.

## 2026-09-19 — 데모 사용자 흐름과 결과 공유 카드

- 분석 API 응답이 UplinkLoader의 첫 100% 이후 도착하면 다음 반복의 100%까지 기다리지 않고 짧은 완료 전환 뒤 결과를 표시한다.
- 홈 제목의 최대 크기와 텍스트 폭을 조정하고 한국어 단어 단위 줄바꿈을 허용해 다양한 화면 폭에서 예시 카드와 겹치지 않게 한다.
- 실제 리포트에서 저장소명·고정 commit·점수·협업 유형·네 축만 담은 1200×630 PNG 결과 카드를 내려받을 수 있다. 파일 공유를 지원하는 브라우저는 Web Share API로 같은 PNG를 공유하며, 미지원 브라우저는 다운로드로 안전하게 대체한다.
- 공유 카드에는 파일 원문·근거 경로·사용자 정보를 넣지 않고 개인 AI 실력 인증이 아니라는 한계를 이미지 안에 표시한다.

## 2026-09-19 — README 리디자인 및 스크린샷 추가

- 라이브 데모에서 실제 화면(홈 히어로, 협업 유형 스펙트럼 카드, 축별 신호 카드)을 캡처해 `docs/assets/readme/`에 추가하고 README에 임베드했다. 합성 텍스트나 가짜 수치는 사용하지 않았다.
- CI 배지(`ci.yml` 워크플로 상태), Next.js/React/TypeScript 버전 배지, 목차, GitHub 알림 콜아웃(`> [!NOTE]`, `> [!WARNING]`)을 추가해 시각적으로 정리했다. 기존 본문 내용·경고 문구·링크는 그대로 유지했다.
- 라이선스 배지는 추가하지 않았다 — 저장소에 아직 `LICENSE` 파일이 없다.
