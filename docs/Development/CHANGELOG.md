...
## 2026-09-21 — 회원가입 없는 공개 리포지토리 리더보드
...
- 같은 저장소를 다시 제출하면 이전 점수를 지우고 최신 재검증 결과로 덮어쓴다("현재 저장소 신호" 원칙 유지).
- 새 영구 저장소와 새 공개 데이터 노출 경로가 걸린 결정이라 [ADR-0024](../Architecture/ADR/0024-opt-in-public-repository-leaderboard.md)로 기록했다.
- 자세한 내용: [Phase-09 세션 로그 2026-09-21](Sessions/Phase-09-Anonymous-Repository-Reports.md#2026-09-21-회원가입-없는-서버-재검증-기반-공개-리더보드)
- 검증: 이 환경에서는 `npm run typecheck`/`npm test`/`npm run build`를 실행하지 못했다(미실행, Windows 전용 `node_modules`). Railway Postgres 생성, `DATABASE_URL` 설정, 마이그레이션 SQL 실행도 이 세션에서 수행하지 않았다 — 사용자가 직접 해야 한다.
...
- 색 매핑은 임의 배정이 아니라 각 차원의 왼쪽 극(D/H/S/F=사람이 미리 설계·기록·확인)과 오른쪽 극(R/P/T/E=자동화·실행 결과 의존) 구분을 그대로 따른다.
- 하단 16유형 카드는 코드 4글자 중 다수 계열 색을 왼쪽 띠로 표시하고(2:2로 갈리면 무채색), 클릭하면 `<details>`로 펼쳐지며 유형 설명 문단이 나오도록 바꿨다.
- 자세한 내용: [Phase-09 세션 로그 2026-09-20](Sessions/Phase-09-Anonymous-Repository-Reports.md#2026-09-20-sjti-유형-8글자에-골드바이올렛-색-구분-추가)
- 검증: 이 환경에서는 `npm run typecheck`/`npm test`/`npm run build`를 실행하지 못했다(미실행, Windows 전용 `node_modules`). 실제 실행은 로컬 환경에서 필요하다.
