...
새 주요 설계 결정의 상세 기록은 [Architecture/ADR](../Architecture/ADR/README.md)에 둔다. 저장소 점수 v2.1의 내용 기반 규칙은 [ADR-0017](../Architecture/ADR/0017-content-aware-repository-scoring.md), 근거 충분성에 따른 네 차원 유형은 [ADR-0018](../Architecture/ADR/0018-evidence-aware-collaboration-profile.md)에 기록한다. 아래 1~7절의 과거 이력은 유지하며, 8~9절의 Phase 4 결정은 ADR-0001~0006으로 정리했다. 세션은 실행 이력, ADR은 선택 이유와 결과를 담당한다.

정본: [마스터 플랜](../00_MASTER_PLAN.md). 과거 참고: [v0.2 원본 마스터](https://github.com/SangJun-Pyo/MyAiScore/blob/f108be37d0df9a8e23a65794ccef0cc367256e0b/_archive/v0.2/00_MASTER_PLAN.original.md). v0.3 초안은 _archive/v0.3.1의 수정 전 ZIP에 보존돼 있다. 2026-09-14 확인 시 루트 v0.2 ZIP은 없어 현재 존재하는 원본 링크로 교체했다.
