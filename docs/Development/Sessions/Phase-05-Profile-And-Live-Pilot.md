...
첫 PR #17 검사(`40f4699`, run `34932225908`)는 23/24 통과, 모바일 모션 전환 1건 실패였다. 로컬 반복도 수정 전 2/10 실패했고, 진단 probe에서 화면에 보이는 장면의 기존 MediaQueryList.matches는 true인데 change 이벤트는 오지 않았음을 확인했다. RAF가 이 값을 보고 애니메이션만 종료해 WebGL 표시 상태가 남는 경로였다.

프레임에서 모션 감소를 감지하면 정적 화면 전환도 수행하고, 탭/장면 복귀 시 현재 선호도를 다시 반영한다. 별도 타이머나 테스트 재시도를 추가하지 않았다. 독립 검토자는 관측 실패에 직접 대응하며 추가 차단 사항이 없음을 확인했다. 역방향 변경 이벤트까지 누락될 때는 같은 화면에서 즉시 감지하지 못하고 화면·탭 복귀 시 회복한다는 한계는 남는다.

최종 build와 전체 **26/26 브라우저 검사**, 수정 후 모바일 기존 시나리오 **10/10 반복**이 통과했다. 신규 2건은 의도적으로 change 이벤트 전달을 차단해 모션 감소 시 정적 전환과 화면 복귀 시 회복을 검증한다. 임시 진단 probe는 제거했다. 원격 재검사 결과는 [PR #17](https://github.com/SangJun-Pyo/MyAiScore/pull/17)의 최종 Checks를 따른다.
