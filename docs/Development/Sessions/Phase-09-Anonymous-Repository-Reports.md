...
해석 가이드에서 `균형 잡힌 빌더`, `검증 수호자`처럼 네 축 분포로 정해지는 결과 스타일 목록이 네 글자 유형 설명 뒤에 가려지던 구조를 수정했다. 점수 규칙 카드 다음에 축 기반 결과 스타일 목록을 항상 표시하고, 그 아래에 네 글자 코드 체계를 설명한다.
...
후속 카피 정리에서 6개 축 기반 스타일명을 `스타터 빌더`, `균형 잡힌 빌더`, `맥락 설계자`, `검증 수호자`, `기록 전략가`, `자동화 지휘자`로 통일했다. SJTI 유형 카드의 설명은 “관찰된 신호로 가장 가까운 유형” 같은 공통 안내 대신 각 유형의 특성을 설명하는 문장으로 바꾸고, 한계 문구는 해석 가이드 하단에 한 번만 표시한다.
...
## 2026-09-20 — SJTI 유형 8글자에 골드·바이올렛 색 구분 추가

해석 가이드와 결과 화면의 SJTI 4글자 코드(D/R, H/P, S/T, F/E)와 하단 16유형 목록이 지금까지 색이 없거나(코드 박스) 살구색 단색(하단 목록 코드)이었다. 살구색·민트는 이미 이 제품에서 "보통/주의"와 "만점"이라는 가치 판단 의미로 쓰이고 있어, 같은 색을 유형 코드에 그대로 재사용하면 특정 SJTI 유형이 "정답"이나 "더 나은 유형"처럼 읽힐 위험이 있었다. 이 위험을 사용자와 먼저 확인한 뒤, 기존 accent/mint 대신 새 토큰 `--gold`(#c2861f)·`--violet`(#8778d6)을 추가해 유형 코드 전용으로 분리했다. `dataviz` 스킬의 `validate_palette.js`로 두 색이 명도 밴드·채도 하한·CVD 색차(정상 시각 ΔE 26)·대비를 모두 통과하는지 확인했다.

8개 극(D/R/H/P/S/T/F/E)은 임의로 나눈 것이 아니라, `REPOSITORY_COLLABORATION_PROFILE_POLES`에서 각 차원의 왼쪽 극(D/H/S/F)이 "사람이 미리 설계·기록·확인하는 신호", 오른쪽 극(R/P/T/E)이 "자동화·실행 결과에 의존하는 신호"로 이미 나뉘어 있다는 점에 근거해 그대로 색을 매핑했다(`REPOSITORY_PROFILE_POLE_TONE`, `repositoryProfilePresentation.ts`). 적용 범위는 상단 요약 배지(`.repo-profile-code`), 코드 박스(`.repo-profile-identity-code`), 스펙트럼 바(왼쪽=골드/오른쪽=바이올렛 고정), 선택된 극 pill(`.repo-profile-dim-top strong`), 하단 16유형 목록 코드 글자까지다.

하단 16유형 카드에는 코드 4글자 중 다수인 계열 색을 왼쪽 띠로 추가했다(`repo-type-gold`/`repo-type-violet`). 2:2로 정확히 갈리는 코드(예: DPSE)는 억지로 한쪽 색을 고르지 않고 무채색 띠로 남겨, 앞서 점수 카드에 적용한 "확실할 때만 색" 원칙과 일관되게 했다. 같은 카드는 `<details>/<summary>`로 접어 두고 클릭하면 `profileDescriptions`에 있던 유형 설명 문단이 펼쳐지도록 했다(카탈로그 함수가 `description` 필드를 새로 반환). 호버 툴팁 대신 접기 방식을 택한 이유는 모바일에서 호버가 아예 동작하지 않고, 같은 페이지의 "미확인 신호" 카드가 이미 같은 `<details>` 패턴을 쓰고 있어 새 인터랙션을 늘리지 않기 때문이다.

이 변경은 색상 토큰 추가와 표시 로직 조정으로, 데이터 계약이나 아키텍처 결정에 해당하지 않아 ADR 없이 세션 로그로만 기록한다(ADR README의 "아키텍처적으로 중요한 결정만" 원칙, 2026-09-20 결과 화면 색상 변경 항목과 동일한 판단).
