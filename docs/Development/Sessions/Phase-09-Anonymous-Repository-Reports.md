...
## 2026-09-20 — SJTI 유형 목록 동시 펼침과 상대 비율 표시 수정 (#72)

해석 가이드 하단의 16개 SJTI 유형은 CSS grid에서 카드마다 독립 `<details>`를 사용했다. 한 카드만 열면 같은 행의 나머지 카드 높이도 늘어나지만 설명은 선택한 카드에만 나타나 빈 공간처럼 보였다. `RepositoryProfileNameGrid`가 하나의 펼침 상태를 공유하도록 바꿔 어느 summary를 누르더라도 16개 설명이 함께 열리고 다시 함께 닫히게 했다. 각 summary의 기본 클릭 동작만 막고 React 상태로 `open`을 제어해 마우스·터치·키보드의 기존 `<summary>` 접근 방식은 유지한다.

SJTI 차원의 `leftStrength`와 `rightStrength`는 양쪽 신호군의 독립 달성률이라 원래부터 합계 1을 보장하지 않는다. UI는 이 값을 상대 비율처럼 `왼쪽 n% · 오른쪽 m%`로 표시하면서 한쪽 트랙만 보색으로 그려, 예를 들어 75%와 58%가 합계 133%로 보일 수 있었다. 저장된 원본 강도, 선택된 pole, 경계 판정과 리포트 계약은 그대로 두고 presentation 계층에서 `left / (left + right)` 비율로 정규화한다. 반올림한 왼쪽 값의 보수를 오른쪽 값으로 사용해 표시 합계를 항상 100%로 고정했고, 막대 폭과 marker도 같은 정규화 값을 사용한다. 양쪽 근거가 모두 0이면 중립적인 50/50으로 표시한다.
