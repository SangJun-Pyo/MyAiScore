...
첫 구현 슬라이스는 공용 경로 matcher, cap 이전 inventory, 위생·큰 소스 후보 집계, v2 전용 신호 예약 선택, 고정 SHA commit summary API와 원문 미보존 집계 분석기를 추가한다. production `repository-report-v1` 점수와 기본 선택 정책은 아직 바꾸지 않는다.

구현·검증 이력은 [Phase 9](../../Development/Sessions/Phase-09-Anonymous-Repository-Reports.md)에 기록한다. 전체 v2 schema, substance detector, 점수, UI, 유형, 코호트는 아직 미구현이다.
