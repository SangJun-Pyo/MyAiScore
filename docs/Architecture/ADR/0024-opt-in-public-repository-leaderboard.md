...
**미검증(이 세션에서 실행 불가):** 이 클라우드 환경은 Windows 전용 `node_modules`(esbuild, @next/swc 등)를 쓰는 실제 개발 환경과 달라 `npm run typecheck`, `npm test`, `npm run build`를 실행하지 못했다. 실제 Railway Postgres 인스턴스에 대한 연결이나 `leaderboard_entries` 마이그레이션 실행도 수행하지 않았다 — 이는 저장소 소유자가 자신의 Railway 프로젝트에서 직접 해야 한다. 위 코드는 검토만 거쳤으며 로컬 빌드·타입체크·테스트로 확인되지 않았다.
