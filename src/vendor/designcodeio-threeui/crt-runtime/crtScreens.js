...
const paintCinematic = (context, width, height, time) => {
    ...
    if (counting) {
        ...
        context.lineWidth = Math.max(1.4, height * 0.0032);
        context.beginPath();
        ...
        for (let tick = 0; tick < 12; tick += 1) {
            ...
            context.lineTo(cx + Math.cos(angle) * radius * 1.10, cy + Math.sin(angle) * radius * 1.10);
            context.stroke();
        }
        context.textAlign = "center";
        context.textBaseline = "middle";
        context.shadowColor = "rgba(255,255,255,0.55)";
        context.shadowBlur = height * 0.030;
        ...
        context.shadowBlur = height * 0.020;
...
const paintBlueScreen = (context, width, height, time) => {
    ...
    if (Math.floor(time * 2) % 2 === 0)
        context.fillRect(left + advance * prompt.length, y + size * 0.08, advance * 0.9, size * 0.96);
    context.shadowBlur = 0;
};
/* ----------------------------------------------------------------- nintendo */
const GLYPHS = {
        ...
        if (rows) {
            for (let row = 0; row < GLYPH_HEIGHT; row += 1) {
                for (let column = 0; column < GLYPH_WIDTH; column += 1) {
                    if (rows[row * GLYPH_WIDTH + column] === "#")
...
function drawCloud(context, x, y, scale) {
    ...
    for (const [dx, dy, w, h] of puffs)
        context.fillRect(Math.round(x + dx * scale), Math.round(y + dy * scale), Math.round(w * scale), Math.round(h * scale));
}
...
const paintNintendo = (context, width, height, time) => {
    ...
    for (const [speed, cloudY, scale] of [[6, 18, 1.25], [3.5, 32, 0.85]]) {
        ...
        for (let copy = 0; copy < 3; copy += 1)
            drawCloud(context, (((time * speed) + (copy * span) / 3) % span) - 50, cloudY + (copy % 2) * 5, scale);
    }
    ...
    for (let y = groundY + 6; y < height; y += 6) {
        ...
        for (let x = (y % 12 === 0 ? 0 : 6); x < width; x += 12)
    ...
    for (const crateX of CRATE_X) {
        ...
        if (progress > 0 && progress < 1)
            lift = Math.max(lift, Math.sin(progress * Math.PI) * JUMP_LIFT);
    }
