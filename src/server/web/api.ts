...
function auth(request: Request): string | null {
  ...
  if (!token) throw new HttpError(401, 'unauthorized', 'Check your result access token.');
  return digest(token);
}
function authorized(db: Database, id: string, ownerHash: string, now: number) {
  const record = db.assessments[id];
  if (!record || record.ownerHash !== ownerHash || Date.parse(record.expiresAt) <= now) throw new HttpError(404, 'not_found', 'Assessment not found.');
  return record;
}
function requireState(record: AssessmentRecord, state: string) {
  if (record.status !== state) throw new HttpError(409, 'invalid_transition', 'This request is not available at the current stage.');
}
...
function useQuota(db: Database, bucket: string, limit: number, now: number) {
  ...
  if ((db.usage[key] ?? 0) >= limit) throw new HttpError(429, 'rate_limited', 'The daily analysis request limit has been reached. Try again later.', true);
...
function safeFailure(error: unknown, stage: string) {
  ...
  return { stage, code: safe, message: safe === 'provider_not_configured' ? 'Live assessment is not configured yet.' : 'This step could not be completed. Check your input and the service status.', retryable };
}
  ...
  return async function handle(request: Request, segments: string[]): Promise<Response> {
    try {
      ...
      if (resource === 'leaderboard') {
        if (method === 'GET') {
          if (id || action) throw new HttpError(404, 'not_found', 'Route not found.');
          if (!leaderboardConfigured()) throw new HttpError(503, 'leaderboard_not_configured', 'Leaderboard storage is not configured yet.');
          const limitParam = new URL(request.url).searchParams.get('limit') ?? undefined;
          const entries = await listLeaderboard(positiveLimit(limitParam, 50));
        ...
        if (method === 'POST') {
          ...
          const normalized = normalizeAndValidateRepoUrl(input.repo_url);
          if (!normalized.ok) throw new HttpError(400, normalized.code, 'https://github.com/소유자/저장소 형식의 공개 저장소 주소를 입력해 주세요.');
          if (await recentlySubmitted(normalized.ref.owner, normalized.ref.repo)) throw new HttpError(429, 'leaderboard_cooldown', '이 저장소는 방금 제출됐어요. 잠시 뒤 다시 시도해 주세요.', true);
          const repoUrl = `https://github.com/${normalized.ref.owner}/${normalized.ref.repo}`;
          const configuredToken = deps.githubToken === undefined ? process.env.GITHUB_TOKEN?.trim() : deps.githubToken?.trim();
          let release: (() => void) | undefined;
          try {
            release = (deps.repositoryReportAdmission ?? repositoryReportAdmission).acquire(Boolean(configuredToken));
            const report = await (deps.repositoryReport ?? generateRepositoryReport)({ repoUrl }, { authToken: configuredToken || undefined });
            await upsertLeaderboardEntry(report);
            return reply(report);
          } catch (error) {
            if (error instanceof RepositoryReportError) throw new HttpError(error.status, error.code, error.message, error.retryable);
            throw error;
      ...
      if (method === 'GET' && resource === 'assessments' && !id && !action) {
        ...
        if (!ownerHash) throw new HttpError(401, 'unauthorized', 'An access token from this browser is required.');
      ...
      if (resource === 'results' && id && !action && method === 'GET') {
        ...
        if (!record?.result) throw new HttpError(404, 'not_found', 'Shared result not found.');
        return reply(publicView(record.result, record.repoUrl));
      }
      if (resource !== 'assessments') throw new HttpError(404, 'not_found', 'Route not found.');
      const ownerHash = auth(request);
      if (method === 'POST' && !id) {
        const rawInput = await bodyOf(request);
        const input = createInput(rawInput);
        const createKey = request.headers.get('idempotency-key');
        if (ownerHash && (!createKey || !/^[A-Za-z0-9_-]{8,100}$/.test(createKey))) throw new HttpError(400, 'invalid_input', 'A valid Idempotency-Key is required.');
        ...
        const created = await store.transaction(db => {
          purgeExpired(db, now());
          if (ownerHash && !Object.values(db.assessments).some(r => r.ownerHash === ownerHash)) throw new HttpError(401, 'unauthorized', 'The access token is not valid.');
          if (createOpKey && db.operations[createOpKey]) {
            const old = db.operations[createOpKey]!;
            if (old.hash !== createHash) throw new HttpError(409, 'idempotency_conflict', 'Different content cannot use the same request key.');
            return old.body;
          }
      ...
      if (!id || !ownerHash) throw new HttpError(401, 'unauthorized', 'The access token from the browser that created this result is required.');
      ...
      if (method === 'GET' && action === 'comparison') {
        if (!initial.result || !initial.previousAssessmentId) throw new HttpError(409, 'comparison_unavailable', 'Comparison requires a previous assessment and a completed current assessment.');
        const previous = authorized(await store.read(), initial.previousAssessmentId, ownerHash, now());
        if (!previous.result) throw new HttpError(409, 'comparison_unavailable', 'The previous assessment is not complete.');
      ...
      if (!key || !/^[A-Za-z0-9_-]{8,100}$/.test(key)) throw new HttpError(400, 'invalid_input', 'A valid Idempotency-Key is required.');
      ...
      if (previous) {
        if (previous.hash !== payloadHash) throw new HttpError(409, 'idempotency_conflict', 'Different content cannot use the same request key.');
        if (previous.state === 'complete') return reply(previous.body, previous.status);
        throw new HttpError(409, 'request_in_progress', 'This request is already in progress. Check its status.');
      }
      ...
      if (isStage) {
        ...
        if (action === 'retry') {
          ...
          else throw new HttpError(409, 'invalid_transition', 'There is no failure available to retry.');
        }
        if (!['ingest', 'questions', 'finalize'].includes(stage)) throw new HttpError(409, 'invalid_transition', 'Check the stage to retry.');
        let provider: EvaluationProvider | undefined;
        if (stage !== 'ingest') {
          if (!(deps.liveEnabled ?? liveConfiguration())) throw new HttpError(503, 'provider_not_configured', 'Live assessment is not configured yet. Explore the example first.');
          try { provider = (deps.provider ?? createAnthropicProviderFromEnv)(); }
          catch { throw new HttpError(503, 'provider_not_configured', 'Check the live assessment service configuration.'); }
        }
        const attemptId = randomUUID();
        const working = await store.transaction(db => {
          const record = authorized(db, id, ownerHash, now());
          if (db.operations[operationKey]) throw new HttpError(409, 'request_in_progress', 'This request is already in progress.');
          if (record.attempt && record.attempt.deadline >= now()) throw new HttpError(409, 'request_in_progress', 'Analysis is already in progress.');
          if (action !== 'retry') {
            requireState(record, stage === 'finalize' ? 'awaiting_answers' : 'draft');
            if (stage === 'questions' && !record.prepared) throw new HttpError(409, 'invalid_transition', 'Collect the repository first.');
          } else if (!(record.status === 'failed' && record.failure?.retryable) && !(record.attempt && record.attempt.deadline < now())) {
            throw new HttpError(409, 'invalid_transition', 'There is no failure available to retry.');
          }
          if (action === 'retry' && stage !== (record.attempt?.stage ?? record.failure?.stage)) throw new HttpError(409, 'stale_attempt', 'The failed stage has changed. Check the status again.');
          if (stage !== 'ingest') {
        ...
        const completed = await store.transaction(db => {
          const record = authorized(db, id, ownerHash, now());
          if (record.attempt?.id !== attemptId || record.revision !== working.revision || record.attempt.deadline < now()) throw new HttpError(409, 'stale_attempt', 'This analysis result has expired. Check the status and try again.');
          record.attempt = null;
      ...
      const completed = await store.transaction(db => {
        const record = authorized(db, id, ownerHash, now());
        if (db.operations[operationKey]) throw new HttpError(409, 'request_in_progress', 'This request is already in progress.');
        if (activeStates.has(record.status) && !(method === 'DELETE' && !action)) throw new HttpError(409, 'request_in_progress', 'Changes are unavailable while analysis is in progress.');
        ...
        if (method === 'PUT' && action === 'answers') {
          ...
          if (answers.some(a => !ids.has(a.questionId))) throw new HttpError(400, 'invalid_input', 'You can only answer questions from this assessment.');
          ...
          if (!['public', 'private'].includes(String(data.visibility))) throw new HttpError(400, 'invalid_input', 'Check the visibility setting.');
      ...
      return reply({ error: { code: 'internal_error', message: 'The request could not be processed. Try again shortly.', retryable: true } }, 500);
    }
  };
}
