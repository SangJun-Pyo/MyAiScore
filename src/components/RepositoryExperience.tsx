...
export type RepositoryReport = {
  ...
  repo: string;
  commitSha: string;
  coverage: { status: "complete" | "partial"; selectedFiles: number; readFiles: number; warnings: string[] };
  score: {
    value: number;
    label: string;
    explanation: string;
    breakdown: { context: number; verification: number; traceability: number; automation: number };
  };
  style: { id: string; title: string; description: string };
  evidence: {
    axis: "context" | "verification" | "traceability" | "automation";
    title: string;
    description: string;
    paths: string[];
  }[];
...
const DEMO_REPORT: RepositoryReport = {
  schemaVersion: "repository-report-v1",
  ruleVersion: "repository-signals-v1",
  source: "github_public_repository",
  repo: "https://github.com/example/sample-project",
  commitSha: "0123456789abcdef0123456789abcdef01234567",
  coverage: { status: "complete", selectedFiles: 18, readFiles: 18, warnings: [] },
  score: {
    value: 72,
    label: "AI 협업 준비도",
    explanation: "저장소에 남은 문서, 검증, 기록, 자동화 신호를 네 축으로 살펴본 예시입니다.",
    breakdown: { context: 20, verification: 18, traceability: 16, automation: 18 },
  },
  style: { id: "steady-builder", title: "차근차근 쌓는 빌더", description: "프로젝트 규칙과 검증 장치를 함께 남기는 경향이 보입니다." },
  evidence: [
    { axis: "context", title: "프로젝트 맥락", description: "작업 규칙과 시작 방법이 문서에 정리되어 있습니다.", paths: ["README.md", "AGENTS.md"] },
    { axis: "verification", title: "검증 장치", description: "자동 테스트와 브라우저 검증 파일이 있습니다.", paths: ["tests/", "playwright.config.ts"] },
    { axis: "traceability", title: "결정 기록", description: "중요한 선택을 다시 확인할 문서가 있습니다.", paths: ["docs/Architecture/ADR/"] },
    { axis: "automation", title: "반복 자동화", description: "CI에서 검사할 작업이 정의되어 있습니다.", paths: [".github/workflows/ci.yml"] },
  ],
  gaps: ["실제 AI 대화에서 어떤 제안을 채택하거나 거절했는지는 저장소만으로 확인할 수 없습니다."],
  nextChallenge: { title: "결정 한 가지를 짧게 기록해 보기", description: "다음 작업에서 선택한 방법과 버린 대안을 한 문단으로 남겨 보세요." },
};
...
function isSafeText(value: unknown, max = 2_000): value is string {
  return typeof value === "string" && value.length > 0 && value.length <= max;
}

function isStringArray(value: unknown, maxItems = 100, maxLength = 1_000): value is string[] {
  return Array.isArray(value) && value.length <= maxItems && value.every(item => isSafeText(item, maxLength));
}

function isBoundedInt(value: unknown, max: number) {
...
export function parseRepositoryReport(value: unknown): RepositoryReport {
  if (!isRecord(value) || !hasExactKeys(value, ["schemaVersion", "ruleVersion", "source", "repo", "commitSha", "coverage", "score", "style", "evidence", "gaps", "nextChallenge"])) throw new Error("invalid_report");
  if (value.schemaVersion !== "repository-report-v1" || value.ruleVersion !== "repository-signals-v1" || value.source !== "github_public_repository") throw new Error("invalid_report");
  if (!isSafeText(value.repo, 500) || !/^https:\/\/github\.com\/[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+\/?$/.test(value.repo)) throw new Error("invalid_report");
  if (typeof value.commitSha !== "string" || !/^[a-f0-9]{40}$/i.test(value.commitSha)) throw new Error("invalid_report");

  const coverage = value.coverage;
  if (!isRecord(coverage) || !hasExactKeys(coverage, ["status", "selectedFiles", "readFiles", "warnings"]) || !["complete", "partial"].includes(String(coverage.status)) || !isBoundedInt(coverage.selectedFiles, 10_000) || !isBoundedInt(coverage.readFiles, 10_000) || Number(coverage.readFiles) > Number(coverage.selectedFiles) || !isStringArray(coverage.warnings, 50)) throw new Error("invalid_report");

  const score = value.score;
  if (!isRecord(score) || !hasExactKeys(score, ["value", "label", "explanation", "breakdown"]) || !isBoundedInt(score.value, 100) || !isSafeText(score.label) || !isSafeText(score.explanation)) throw new Error("invalid_report");
  const breakdown = score.breakdown;
  if (!isRecord(breakdown) || !hasExactKeys(breakdown, ["context", "verification", "traceability", "automation"]) || !Object.values(breakdown).every(item => isBoundedInt(item, 25)) || Object.values(breakdown).reduce<number>((sum, item) => sum + Number(item), 0) !== score.value) throw new Error("invalid_report");

  const style = value.style;
  if (!isRecord(style) || !hasExactKeys(style, ["id", "title", "description"]) || !isSafeText(style.id, 100) || !isSafeText(style.title) || !isSafeText(style.description)) throw new Error("invalid_report");
  if (!Array.isArray(value.evidence) || value.evidence.length > 100) throw new Error("invalid_report");
  const seenAxes = new Set<string>();
  for (const item of value.evidence) {
    if (!isRecord(item) || !hasExactKeys(item, ["axis", "title", "description", "paths"]) || !AXES.some(axis => axis.id === item.axis) || !isSafeText(item.title) || !isSafeText(item.description) || !isStringArray(item.paths, 100, 500)) throw new Error("invalid_report");
    seenAxes.add(String(item.axis));
  }
  if (!AXES.every(axis => seenAxes.has(axis.id)) || !isStringArray(value.gaps, 100)) throw new Error("invalid_report");
  const nextChallenge = value.nextChallenge;
  if (!isRecord(nextChallenge) || !hasExactKeys(nextChallenge, ["title", "description"]) || !isSafeText(nextChallenge.title) || !isSafeText(nextChallenge.description)) throw new Error("invalid_report");
  return value as RepositoryReport;
}
...
function RepositoryReportView({ report, demo = false, actions = true }: { report: RepositoryReport; demo?: boolean; actions?: boolean }) {
  ...
  return <section className="repo-report" aria-label="저장소 분석 리포트">
    ...
    <section className="repo-axis-section" aria-labelledby="axis-heading"><div className="repo-section-heading"><div><span className="eyebrow">네 가지 저장소 신호</span><h2 id="axis-heading">어떤 흔적이 점수에 반영됐나요?</h2></div><p>{report.coverage.readFiles}/{report.coverage.selectedFiles}개 선택 파일 확인 · {report.coverage.status === "complete" ? "수집 완료" : "일부 수집"}<br />파일에 남은 정적 신호만 반영</p></div><div className="repo-axis-grid">{AXES.map(axis => { const item = report.evidence.find(entry => entry.axis === axis.id)!; return <article className="repo-axis-card product-panel" key={axis.id}><div className="repo-axis-top"><span>{axis.name}</span><strong>{report.score.breakdown[axis.id]}<small>/25</small></strong></div><h3>{item.title}</h3><p>{item.description}</p><details><summary>근거 파일 {item.paths.length}개</summary>{item.paths.length ? <ul>{item.paths.map(path => <li key={path}><code>{path}</code></li>)}</ul> : <p>이 축에서 표시할 근거 경로가 없습니다.</p>}</details><p className="caption">{axis.question}</p></article>; })}</div></section>
    <div className="repo-detail-grid"><section className="product-panel repo-gaps"><span className="eyebrow">확인할 수 없었던 것</span><h2>빈칸도 결과의 일부입니다.</h2>{report.gaps.length ? <ul>{report.gaps.map((gap, index) => <li key={index}>{gap}</li>)}</ul> : <p>이번 규칙에서 별도로 표시된 빈칸이 없습니다.</p>}{report.coverage.warnings.length > 0 && <details><summary>수집 경고 {report.coverage.warnings.length}개</summary><ul>{report.coverage.warnings.map((warning, index) => <li key={index}>{warning}</li>)}</ul></details>}</section><section className="repo-next"><span className="eyebrow">다음 도전</span><h2>{report.nextChallenge.title}</h2><p>{report.nextChallenge.description}</p></section></div>
...
export function RepositoryEvaluateExperience() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setStatus("error"); setMessage(cause instanceof Error && cause.message !== "invalid_report" ? cause.message : "서버 응답을 확인할 수 없습니다. 잠시 후 다시 시도해 주세요.");
    }
  }
