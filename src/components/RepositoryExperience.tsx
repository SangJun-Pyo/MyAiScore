...
function ProfileCode({ code, className }: { code: string; className: string }) {
  return <strong className={className}>{code.split("").map((letter, index) => <span key={`${letter}-${index}`} className={poleToneClass(letter)}>{letter}</span>)}</strong>;
}

function RepositoryProfileNameGrid({ items }: { items: ReturnType<typeof repositoryProfileNameCatalog> }) {
  const [expanded, setExpanded] = useState(false);
  return <div className="repo-profile-name-grid">{items.map(item => <div key={item.code}>
    <div className="repo-profile-name-tone" aria-hidden="true">{item.code.split("").map((letter, index) => <b key={`${letter}-${index}`} className={poleToneClass(letter)} />)}</div>
    <details open={expanded}><summary onClick={event => { event.preventDefault(); setExpanded(value => !value); }}><ProfileCode code={item.code} className="repo-profile-name-code" /><span>{item.name}</span></summary><p>{item.description}</p></details>
  </div>)}</div>;
}
...
function RepositoryReportView({ report, demo = false, actions = true, overviewVariant = "guide" }: { report: RepositoryReport; demo?: boolean; actions?: boolean; overviewVariant?: "default" | "guide" }) {
  ...
  return <section className="repo-report" aria-label={copy.report.aria}>
    ...
    {guideOverview ? <section className="repo-overview-guide repo-hero-card product-panel" aria-label={display.scoreLabel}>
      ...
      <aside className="repo-overview-guide-side">
        {profileDisplay && <div className="repo-overview-profile-summary">
          ...
          <div>{profileDisplay.code && <ProfileCode code={profileDisplay.code} className="repo-profile-code" />}<b>{profileDisplay.title}</b></div>
    ...
    {profileDisplay && <section className="product-panel repo-profile-panel" aria-label={profileDisplay.guideTitle}>
      ...
      {profileDisplay.code && <div className="repo-profile-identity"><ProfileCode code={profileDisplay.code} className="repo-profile-identity-code" /><div className="repo-profile-identity-text"><strong>{profileDisplay.title}</strong></div></div>}
      <div className="repo-profile-dimensions">{profileDisplay.dimensions.map(dimension => { const leftSelected = dimension.selectedPole === dimension.leftPole; const rightSelected = dimension.selectedPole === dimension.rightPole; return <article key={dimension.id}><div className="repo-profile-dim-top"><h3>{dimension.title}</h3><strong className={leftSelected ? "is-left" : rightSelected ? "is-right" : undefined}>{dimension.selectedLabel}</strong></div><p>{dimension.description}</p><div className="repo-profile-spectrum"><div className="repo-profile-spectrum-labels"><span className={leftSelected ? "is-selected" : undefined}>{dimension.leftPole} · {dimension.leftLabel}</span><span className={rightSelected ? "is-selected" : undefined}>{dimension.rightPole} · {dimension.rightLabel}</span></div><div className="repo-profile-spectrum-track">{leftSelected && <div className="repo-profile-spectrum-fill is-left" style={{ width: `${dimension.leftPercent}%` }} />}{rightSelected && <div className="repo-profile-spectrum-fill is-right" style={{ width: `${dimension.rightPercent}%` }} />}<div className="repo-profile-spectrum-marker" style={{ left: `${dimension.leftPercent}%` }} /></div></div><small>{profileDisplay.strength(dimension.leftPercent, dimension.rightPercent)}{dimension.boundaryLabel ? ` · ${dimension.boundaryLabel}` : ""}</small></article>; })}</div>
    </section>}
    <section className="repo-axis-section" aria-labelledby="axis-heading">
      ...
      <div className="repo-axis-grid">{REPOSITORY_AXIS_ORDER.map(axis => {
        ...
        const axisValue = report.score.axes[axis].value;
        const axisIsFull = axisValue >= 25;
        return <article className={`repo-axis-card product-panel${axisIsFull ? " repo-axis-card-full" : ""}`} key={axis}>
          <div className="repo-axis-top"><div className="repo-axis-identity"><span className="repo-axis-ring" style={{ background: `conic-gradient(${axisIsFull ? "var(--mint)" : "var(--accent)"} ${(axisValue / 25) * 360}deg, var(--line-strong) 0deg)` }}><span className="repo-axis-ring-inner">{AXIS_ICONS[axis]}</span></span><span>{copy.presentation.axes[axis].label}</span></div><strong>{axisValue}<small>/25</small></strong></div>
          {cards.length ? cards.map(card => { const cardCopy = display.evidence[card.id]; const assessment = v2?.signalScores.find(item => item.id === card.id); const isFullScore = !!assessment && assessment.points >= assessment.maxPoints; return <div key={card.id} className="repo-evidence-card"><h3>{cardCopy.title}</h3>{assessment && <strong className={`repo-signal-score${isFullScore ? " repo-signal-score-full" : ""}`}>{assessment.substance === null ? v2Display.unmeasuredQuality(assessment.points, assessment.maxPoints) : v2Display.measuredQuality(assessment.points, assessment.maxPoints, Math.round(assessment.substance * 100))}</strong>}<p>{cardCopy.description}</p><details><summary>{copy.report.evidenceFiles(card.paths.length)}</summary><ul>{card.paths.map(path => <li key={path}><code>{path}</code></li>)}</ul></details></div>; }) : <p>{copy.report.noSignal}</p>}
          {axis === "traceability" && v2 && (() => { const assessment = v2.signalScores.find(item => item.id === "traceability-commit-practice"); if (!assessment?.presence) return null; const isFullScore = assessment.points >= assessment.maxPoints; return <div className="repo-evidence-card"><h3>{v2Display.commitPractice}</h3><strong className={`repo-signal-score${isFullScore ? " repo-signal-score-full" : ""}`}>{v2Display.measuredQuality(assessment.points, assessment.maxPoints, Math.round((assessment.substance ?? 0) * 100))}</strong><p>{v2Display.commitPracticeDescription}</p></div>; })()}
          <p className="caption">{copy.presentation.axes[axis].question}</p>
        </article>;
      })}</div>
    </section>
    <div className="repo-detail-grid">
      <section className={`product-panel repo-gaps${display.gaps.length ? "" : " repo-gaps-clear"}`}><span className="eyebrow">{copy.report.gapsEyebrow}</span><h2>{copy.report.gapsHeading}</h2>{display.gaps.length ? <ul>{display.gaps.map((gap, index) => <li key={index}>{gap}</li>)}</ul> : <p>{copy.report.noGaps}</p>}<details><summary>{copy.report.coverageDetails}</summary><p>{display.coverageNote}</p>{report.coverage.treeTruncated && <p>{copy.report.treeTruncated}</p>}{report.coverage.selectionLimited && <p>{copy.report.selectionLimited}</p>}</details></section>
...
function RepositoryInterpretationGuide({ report }: { report: RepositoryReport | null }) {
  ...
  return <div className="repo-interpretation-guide">
    ...
    {profileGuide && <section className="repo-style-guide" aria-labelledby="profile-guide-heading">
      ...
      <div className="repo-profile-dimensions">{profileGuide.dimensions.map(dimension => { const leftSelected = dimension.selectedPole === dimension.leftPole; const rightSelected = dimension.selectedPole === dimension.rightPole; return <article key={dimension.id}><div className="repo-profile-dim-top"><h3>{dimension.title}</h3><strong className={leftSelected ? "is-left" : rightSelected ? "is-right" : undefined}>{dimension.selectedLabel}</strong></div><p>{dimension.description}</p><div className="repo-profile-spectrum"><div className="repo-profile-spectrum-labels"><span className={leftSelected ? "is-selected" : undefined}>{dimension.leftPole} · {dimension.leftLabel}</span><span className={rightSelected ? "is-selected" : undefined}>{dimension.rightPole} · {dimension.rightLabel}</span></div><div className="repo-profile-spectrum-track">{leftSelected && <div className="repo-profile-spectrum-fill is-left" style={{ width: `${dimension.leftPercent}%` }} />}{rightSelected && <div className="repo-profile-spectrum-fill is-right" style={{ width: `${dimension.rightPercent}%` }} />}<div className="repo-profile-spectrum-marker" style={{ left: `${dimension.leftPercent}%` }} /></div></div><small>{profileGuide.strength(dimension.leftPercent, dimension.rightPercent)}{dimension.boundaryLabel ? ` · ${dimension.boundaryLabel}` : ""}</small></article>; })}</div>
      <section className="repo-profile-name-guide" aria-labelledby="profile-name-guide-heading"><div className="repo-style-guide-heading"><div><span className="eyebrow">{copy.insights.profileNamesEyebrow}</span><h2 id="profile-name-guide-heading">{copy.insights.profileNamesTitle}</h2></div><p>{copy.insights.profileNamesIntro}</p></div><RepositoryProfileNameGrid items={profileNames} /></section>
    </section>}
    {!profileGuide && <section className="repo-style-guide" aria-labelledby="style-name-guide-heading">
      <section className="repo-profile-name-guide" aria-labelledby="style-name-guide-heading"><div className="repo-style-guide-heading"><div><span className="eyebrow">{copy.insights.profileNamesEyebrow}</span><h2 id="style-name-guide-heading">{copy.insights.profileNamesTitle}</h2></div><p>{copy.insights.profileNamesIntro}</p></div><RepositoryProfileNameGrid items={profileNames} /></section>
    </section>}
