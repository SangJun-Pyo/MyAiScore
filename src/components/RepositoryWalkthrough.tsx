  ...
  return <Shell><main id="main" className="page-width product-page assessment-page">
    ...
    {data && <>
      {stage === 0 && <section className="product-panel assessment-form" aria-labelledby="snapshot-heading">
        <h2 id="snapshot-heading">Repository snapshot</h2>
        ...
        {(data.collection.selection_limited || data.collection.context_truncated) && <div className="notice"><strong>Limited evidence coverage</strong><p>{data.collection.selection_limited && "Only a sample of eligible files was selected. "}{data.collection.context_truncated && "Some collected content was shortened for the assessment context."}</p></div>}
      ...
      {stage === 1 && <section className="questions-panel" aria-labelledby="walkthrough-questions-heading">
        <div className="section-heading"><div><h2 id="walkthrough-questions-heading">3 scripted questions</h2><p>Written for this snapshot. No service model generated these questions.</p></div></div>
      ...
      {stage === 2 && <section aria-label="Scripted walkthrough report"><ResultView assessment={data.assessment} /><div className="button-row"><button type="button" className="button button-secondary" onClick={() => setStage(1)}>Back to scripted questions</button></div></section>}
