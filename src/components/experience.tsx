...
export function ProfileExperience() {
  ...
  return <Shell><main id="main" className="page-width workspace-page product-page profile-page">
    ...
    {!loading && !error && <>
      <div className="product-summary" aria-label="Assessment counts"><span><strong>{history?.total ?? 0}</strong> saved</span><span><strong>{completed.length}</strong> completed in this list</span><span><strong>{entries.length - completed.length}</strong> to continue</span></div>
...
export function InsightsExperience() {
  ...
  return <Shell><main id="main" className="page-width workspace-page product-page insights-page"><header className="product-heading"><div><h1>Insights</h1><p>Explore the evidence behind each assessment dimension.</p></div><Link href="/profile" className="text-button">Back to profile <Arrow /></Link></header>
    ...
    {mode === "own" && !loadingHistory && !historyError && !selected && completed.length === 0 && <div className="insight-empty product-empty"><span className="empty-small-icon" aria-hidden="true">◎</span><div><strong>No completed assessments to show.</strong><p>Explore the criteria below. Examples are never substituted for your results automatically.</p></div><Link href="/evaluate" className="button button-secondary">Start your first assessment <Arrow /></Link></div>}
    ...
    <section id="insight-panel" role="tabpanel" aria-labelledby={`insight-tab-${axisCode}`} tabIndex={0} className="insight-panel product-panel"><div className="insight-axis-heading"><div><span className="axis-kicker">DIMENSION {axisCode} / 05</span><h2>{axis.name}</h2><p>{rubric.question}</p></div><div className="axis-weight"><strong>{axis.weight}<small>%</small></strong><span>Weight in the total score</span></div></div><div className="insight-columns"><div className="insight-observation"><div className="workspace-subheading"><h3>{mode === "example" ? "Example observations" : "This project's observations"}</h3><span className={`level-badge ${observed ? "" : "unobserved"}`}>{assessment?.status === "done" ? criterionLabel(criterion, true) : assessment ? "In progress" : "No assessment"}</span></div>
