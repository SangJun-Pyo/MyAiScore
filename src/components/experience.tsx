...
export function ProfileExperience() {
  ...
  return <Shell><main id="main" className="page-width workspace-page"><div className="workspace-heading"><div><SectionLabel>YOUR COLLABORATION SPACE</SectionLabel><h1>Your project's next chapter.</h1><p>Keep a record of the process. Bring clarity to your next decision.</p></div><Link className="button button-primary" href="/evaluate">Start an assessment <Arrow /></Link></div>
    <section className="profile-identity"><div className="profile-avatar" aria-hidden="true"><span>◫</span><i /></div><div><h2>Your collaboration profile <span>This tab</span></h2><p>Project assessments started in this tab. This is not a signed-in account or a certification of personal ability.</p></div><span className="private-chip">Private workspace</span></section>
    ...
    {!loading && !error && <><div className="profile-stats"><article><span>Saved assessments</span><strong>{history?.total ?? 0}<small></small></strong><p>Unexpired records accessible in this tab</p></article><article><span>Completed in this list</span><strong>{completed.length}<small></small></strong><p>Includes results with withheld scores</p></article><article><span>Assessments to continue</span><strong>{entries.filter(entry => entry.status !== "done").length}<small></small></strong><p>Unfinished records in the recent list</p></article></div>
    {entries.length === 0 ? <section className="workspace-empty"><div className="empty-orbit" aria-hidden="true"><span>+</span></div><span className="tiny-label">YOUR FIRST CHAPTER</span><h2>No assessments yet.</h2><p>Connect a project to start your collaboration history.<br />Missing records are never replaced with invented scores.</p><div className="button-row"><Link href="/evaluate" className="button button-primary">Assess your first project <Arrow /></Link><Link href="/insights?view=example" className="button button-secondary">Explore a synthetic example</Link></div></section> : <div className="profile-content"><section className="history-section"><div className="workspace-subheading"><h2>Assessment history</h2><span>{entries.length} recent records</span></div><div className="history-list">{entries.map(entry => <Link key={entry.assessment_id} href={`/assessments/${encodeURIComponent(entry.assessment_id)}`} className="history-entry"><div className="history-repo-icon" aria-hidden="true">⌘</div><div className="history-description"><h3>{repoName(entry.repo_url)}</h3><p>{dateLabel(entry.created_at)}{entry.commit_sha && <code>{entry.commit_sha.slice(0, 8)}</code>}</p><span className={`history-status status-${entry.status}`}>{statusLabel(entry.status)}{entry.visibility === "public" ? " · Summary shared" : ""}</span></div><div className="history-score"><strong>{scoreLabel(entry.score)}</strong>{entry.score?.status === "issued" && <small>/100</small>}</div><span className="history-arrow" aria-hidden="true">↗</span></Link>)}</div>{history?.has_more && <p className="caption">Showing the latest {entries.length} of {history.total} records. Completed and unfinished counts refer to this list.</p>}</section><aside className="profile-latest"><SectionLabel>LATEST COMPLETED REVIEW</SectionLabel><h2>Five perspectives on your latest review</h2>{latest ? <><p className="latest-project">{repoName(latest.repo_url)}</p><div className="latest-axes">{AXES.map(axis => { const criterion = latest.criteria.find(item => item.criterion_code === axis.code); const level = criterion?.status === "observed" ? criterion.level : null; return <Link key={axis.code} href={`/insights?assessment=${encodeURIComponent(latest.assessment_id)}&axis=${axis.code}`}><span>{axis.code}</span><strong>{axis.name}</strong><div className="level-ticks" aria-hidden="true">{[1, 2, 3, 4].map(tick => <i key={tick} className={typeof level === "number" && tick <= level ? "filled" : ""} />)}</div><small>{level === null || level === undefined ? "Not observed" : `Level ${level}`}</small></Link>; })}</div><Link className="text-button" href={`/insights?assessment=${encodeURIComponent(latest.assessment_id)}`}>Explore the evidence by dimension <Arrow /></Link><p className="caption">This is one project's result, not an average or a measure of overall ability.</p></> : <><p>No completed assessments yet. Finish an assessment to see its observed dimensions here.</p><Link className="text-button" href="/insights">Explore the assessment criteria <Arrow /></Link></>}</aside></div>}
    <div className="workspace-privacy"><span aria-hidden="true">↳</span><p>Access is stored in this tab. Closing it or clearing browser data may remove your access. Expired or deleted assessments disappear from this list.</p></div></>}
  </main></Shell>;
}

const AXIS_ACTIONS: Record<string, string> = {
  A: "Before starting, write down the problem, constraints, and observable completion criteria.",
  B: "Share relevant files and task boundaries, then check the AI's understanding of the scope.",
  C: "Record one tool's purpose, an alternative, and why it fit the actual constraints.",
  D: "Reproduce an important failure case and connect the original result to your check after the fix.",
  E: "Record why you accepted, rejected, or changed an AI suggestion and what happened next.",
};
...
export function InsightsExperience() {
  ...
  return <Shell><main id="main" className="page-width workspace-page insights-page"><div className="workspace-heading"><div><SectionLabel>UNDERSTAND YOUR COLLABORATION</SectionLabel><h1>Five perspectives. Deeper understanding.</h1><p>Understand the criteria, then explore the evidence from your project.</p></div><Link href="/profile" className="button button-secondary">Back to your records <Arrow /></Link></div>
    ...
    {mode === "own" && !loadingHistory && !historyError && !selected && completed.length === 0 && <div className="insight-empty"><span className="empty-small-icon" aria-hidden="true">◎</span><div><strong>No completed assessments to show.</strong><p>Explore the criteria below. Examples are never substituted for your results automatically.</p></div><Link href="/evaluate" className="button button-secondary">Start your first assessment <Arrow /></Link></div>}
    ...
    <section id="insight-panel" role="tabpanel" aria-labelledby={`insight-tab-${axisCode}`} tabIndex={0} className="insight-panel"><div className="insight-axis-heading"><div><span className="axis-kicker">DIMENSION {axisCode} / 05</span><h2>{axis.name}</h2><p>{rubric.question}</p></div><div className="axis-weight"><strong>{axis.weight}<small>%</small></strong><span>Weight in the total score</span></div></div><div className="insight-columns"><div className="insight-observation"><div className="workspace-subheading"><h3>{mode === "example" ? "Example observations" : "This project's observations"}</h3><span className={`level-badge ${observed ? "" : "unobserved"}`}>{observed ? `${criterion!.level} / 4` : assessment?.status === "done" ? "Not observed" : assessment ? "In progress" : "No assessment"}</span></div>
      ...
      <div className="insight-next"><span className="tiny-label">A PRACTICAL NEXT STEP</span><h3>Evidence to capture next</h3><p>{AXIS_ACTIONS[axisCode]}</p><small>A general suggestion for this dimension. Adding more material does not guarantee a higher score.</small></div>{mode === "own" && assessment?.assessment_id && <Link className="text-button" href={`/assessments/${encodeURIComponent(assessment.assessment_id)}`}>Open the full review and personalized task <Arrow /></Link>}</div>
      <aside className="rubric-guide"><div className="rubric-guide-heading"><span className="tiny-label">THE EVALUATION GUIDE</span><h3>How this dimension is assessed.</h3><p>Levels 1–4 are experimental criteria under calibration. Missing evidence is a separate state, not a level.</p></div><ol>{rubric.levels.map(level => <li key={level.level} className={observed && criterion!.level === level.level ? "current-level" : ""}><div><span>{level.level}</span><strong>Level {level.level}</strong>{observed && criterion!.level === level.level && <small>{mode === "example" ? "Example" : "This review"}</small>}</div><p>{level.behavior}</p><details><summary>What to keep in mind</summary><p>{level.counterExample.replaceAll("not_observed", "not observed")}</p></details></li>)}</ol></aside></div></section>
  </main></Shell>;
}
...
export function HomeExperience() {
  ...
  return <Shell><main id="main">
    <section className="hero page-width"><div className="hero-copy"><SectionLabel>YOUR NEXT BETTER COLLABORATION</SectionLabel><h1>Built with AI?<br />Now, look at <em>how</em>  you built it.</h1><p className="hero-description">Your decisions go beyond the code.<br />Find strengths in your project and collaboration evidence,<br className="mobile-break" />  and one thing to do differently next time.</p><div className="hero-actions"><a className="button button-primary" href="/evaluate">Review my project <Arrow /></a><button className="button button-text" onClick={() => { void loadExample(); document.getElementById("example")?.scrollIntoView({ behavior: "smooth" }); }}>Preview a result <Arrow diagonal /></button></div><div className="hero-trust"><span><i />Read-only public repositories</span><span>Private results by default</span><span>No signup required</span></div></div>
    <div className="hero-visual hero-visual--logic-core" aria-label="A visualization of connections between AI collaboration and evidence">
      ...
      <div className="hero-axis-controls" role="group" aria-label="Choose a dimension to visualize">{AXES.map(axis => <button key={axis.code} type="button" aria-pressed={activeAxis === axis.code} aria-label={`${axis.code} ${axis.name}`} onClick={() => setActiveAxis(axis.code)}><span>{axis.code}</span><span>{axis.name}</span></button>)}</div>
    </div></section>
    <section className="principle-strip"><div className="page-width"><p>More than how much AI you use,<br /><strong>we look at the decisions you make.</strong></p><div><span className="strip-number">01</span><span>Evidence-led feedback</span></div><div><span className="strip-number">02</span><span>Unknown stays unknown</span></div><div><span className="strip-number">03</span><span>A practical next step</span></div></div></section>
    <section className="section page-width" id="how"><div className="section-heading"><div><SectionLabel>WHAT WE LOOK AT</SectionLabel><h2>Five perspectives on<br />working with AI.</h2></div><p>We distinguish the finished code from the collaboration process.<br />Token counts and the number of tools do not earn extra points.</p></div><div className="axes-grid">{AXES.map(axis => <Link href={`/insights?axis=${axis.code}`} className="axis-intro" key={axis.code}><div className="axis-top"><span className="axis-letter">{axis.code}</span><span>{axis.weight}%</span></div><p className="axis-short">{axis.short}</p><h3>{axis.name}</h3><p>{axis.description}</p></Link>)}</div><p className="caption">Weights and levels 1–4 are experimental and under calibration. A total score is issued only when all five dimensions can be assessed.</p></section>
    <section className="workspace-preview page-width" aria-label="Explore your collaboration records"><Link href="/profile"><span className="workspace-card-icon" aria-hidden="true">◫</span><div><span className="tiny-label">YOUR WORKSPACE</span><h3>A record for every project.</h3><p>Continue the assessments you started in this tab.</p></div><Arrow diagonal /></Link><Link href="/insights"><span className="workspace-card-icon" aria-hidden="true">◎</span><div><span className="tiny-label">FIVE PERSPECTIVES</span><h3>Read the evidence behind the score.</h3><p>Explore the criteria alongside your own assessment.</p></div><Arrow diagonal /></Link></section>

    <section id="example" className="section page-width"><div className="section-heading"><div><SectionLabel>FROM INSIGHT TO ACTION</SectionLabel><h2>After the score,<br />a clear next step.</h2></div><p>This walkthrough uses synthetic material.<br />It does not demonstrate the accuracy of a real model.</p></div>{!example && <div className="example-prompt"><span className="example-symbol" aria-hidden="true">↗</span><div><h3>From evidence to an improvement task</h3><p>Explore a full result without signing up or submitting your data.</p></div><button className="button button-primary" disabled={loadingExample} onClick={() => void loadExample()}>{loadingExample ? "Loading example…" : "Open the synthetic result"}<Arrow /></button></div>}{exampleError && <Alert error>{exampleError}</Alert>}{example && <ResultView assessment={example} example />}</section>
  </main></Shell>;
}
...
export function EvaluateExperience() {
  ...
  return <Shell><main id="main" className="evaluate-page"><div className="page-width evaluate-breadcrumb"><Link href="/profile">My records</Link><span aria-hidden="true">/</span><span>New assessment</span></div>    <section className="start-section" id="start"><div className="page-width start-grid"><div className="start-copy"><SectionLabel>START WITH ONE PROJECT</SectionLabel><h1>One project.<br />A better next step.</h1><p>Start with a public GitHub repository<br />and one memorable AI collaboration case.</p><ol className="steps"><li><span>1</span><div><strong>Connect a project and a case</strong><p>Tell us what you built and which decisions you made.</p></div></li><li><span>2</span><div><strong>Three grounded questions</strong><p>Fill in the process that code alone cannot reveal.</p></div></li><li><span>3</span><div><strong>Review the findings and next step</strong><p>Explore the evidence and take away an improvement task.</p></div></li></ol><div className="privacy-note"><span aria-hidden="true">↳</span><p>We do not execute or modify the repository's code.<br />Only the material you select is used in the assessment.</p></div></div><AssessmentForm config={config} configError={configError} /></div></section><div className="page-width evaluate-example-link"><p>Want to see what a result looks like first?</p><Link href="/?view=example#example" className="text-button">Preview a synthetic result <Arrow diagonal /></Link></div></main></Shell>;
}
...
function ComparisonView({ id, previousId }: { id: string; previousId: string }) {
  ...
  const levelLabel = (value: { status: string; level: number | null } | null | undefined) => value?.status === "observed" && value.level !== null ? `Level ${value.level}` : "Not observed";
