...
export function HomeExperience() {
  ...
  return <Shell><main id="main">
    ...
    <div className="hero-visual" aria-label="가상 결과 카드: 근거를 확인하고 다음 개선 행동을 제안합니다"><div className="visual-top"><span className="tiny-label">COLLABORATION REVIEW</span><span className="sample-chip">가상 예시</span></div><div className="visual-title">잘 만든 결과 뒤의<br /><strong>좋은 판단을 발견하세요.</strong></div><div className="sample-bars">{AXES.map((axis, index) => <div className="sample-row" key={axis.code}><span>{axis.name}</span><div className="bar-track"><div style={{ width: `${[75, 100, 75, 50, 75][index]}%` }} /></div><span className="sample-level">{[3, 4, 3, 2, 3][index]}단계</span></div>)}</div><div className="visual-bottom"><div className="spark-icon" aria-hidden="true">✳</div><div><span className="tiny-label">YOUR NEXT MOVE</span><p>중요한 실패 상황을<br /><strong>직접 재현해 검증하기</strong></p></div><Arrow diagonal /></div><p className="visual-disclaimer">화면 형식을 보여주는 예시이며 실제 평가 결과가 아닙니다.</p></div></section>
