...
const ko = {
  ...
  report: {
    ...
    gapsEyebrow: "확인할 수 없었던 것", gapsHeading: "빈칸도 결과의 일부예요.", noGaps: "이번 규칙에서 따로 표시된 빈칸은 없어요.",
    coverageDetails: "수집 범위 설명", treeTruncated: "GitHub가 파일 트리를 일부만 반환했어요.", selectionLimited: "후보 파일이 많아서 정해진 한도만큼만 골라 읽었어요.",
    nextChallenge: "다음 단계", recommendations: "효율이 높은 다음 작업", recommendationsIntro: "점수 상승 여지와 예상 작업량을 함께 계산해 우선순위를 정했습니다. 계산 계수는 화면에 표시하지 않습니다.", saveIntro: "저장은 선택이에요. 원문이나 GitHub 계정 정보 없이 이 요약만 지금 쓰는 브라우저에 저장돼요.",
    save: "이 브라우저에 저장", insightsLink: "점수 해석 보기 →", saved: "이 브라우저의 ‘내 리포트’에 저장했어요.", saveError: "브라우저 저장 공간에 리포트를 저장하지 못했어요.",
  },
  home: {
    ...
    steps: [{ title: "저장소 주소를 입력합니다.", text: "github.com/소유자/저장소 형식의 공개 GitHub 저장소 주소만 있으면 됩니다." }, { title: "네 가지 신호를 확인합니다.", text: "맥락, 검증 기반, 기록, 자동화에 해당하는 파일과 설정을 살펴봅니다." }, { title: "점수와 근거를 함께 확인합니다.", text: "점수만 보여주는 것이 아니라 평가에 반영된 파일과 확인되지 않은 항목까지 함께 보여드립니다." }],
  ...
  insights: {
    title: "해석 가이드", description: "점수를 만드는 네 축과, 이 점수로는 알 수 없는 것을 정리했어요.", savedReport: "저장된 리포트", choose: "저장소 선택", emptyTitle: "아직 고른 리포트가 없어요.", emptyText: "저장소를 분석한 뒤 ‘점수 해석 보기’를 누르거나, 저장해 둔 리포트를 골라 주세요.", emptyCta: "저장소 분석하기 →", guideEyebrow: "점수 읽는 법", guideTitle: "각 축은 최대 25점이에요.", range: "0–25점", guideText: "총점은 저장소에서 어떤 신호가 보였는지를 나타내요. 점수가 높다고 결과물이 더 좋거나 AI를 더 잘 쓴다는 뜻은 아니에요.",
    matrixEyebrow: "점수 규칙", matrixTitle: "어떤 신호에 몇 점인지 전부 적어 뒀어요.", matrixIntro: "같은 신호는 여러 번 나와도 한 번만 더하고, 축마다 25점에서 더 올라가지 않아요. 같은 저장소의 같은 커밋이면 언제 다시 돌려도 같은 점수가 나와요.", matrixCaption: "맥락·검증 기반·기록·자동화 네 축의 신호별 배점", matrixScrollLabel: "네 축의 신호별 배점표", axisHeader: "축", signalHeader: "저장소 신호", pointsHeader: "점수", axisTotal: "축 합계", points: (value: number) => `${value}점`,
    stylesEyebrow: "협업 스타일 규칙", stylesTitle: "스타일은 총점이 아니라 네 축의 분포로 정해져요.", stylesIntro: "아래 규칙을 위에서부터 확인해서, 처음 맞는 하나로 정해요.", ruleStep: (value: string) => `규칙 ${value}`, firstSignalsRule: (threshold: number) => `총점이 ${threshold}점보다 낮으면 이 스타일이에요.`, balancedRule: (minimum: number, spread: number) => `네 축이 모두 ${minimum}점 이상이고, 가장 높은 축과 가장 낮은 축의 차이가 ${spread}점 이하면 이 스타일이에요.`, dominantRule: (axis: string) => `${axis} 축이 가장 높으면 이 스타일이에요.`, tiePriority: (axes: string) => `가장 높은 축이 여러 개면 이 순서로 정해요 — ${axes}`, activeStyle: "지금 보고 있는 리포트", distributionNote: "이 리포트의 스타일도 총점이 아니라 네 축의 분포로 정해졌어요.",
  },
  presentation: {
