...
로그인이나 CLI 없이 공개 저장소 URL 하나를 입력하면, MyAiScore가 고정 commit의 제한된 파일 표본과 최근 조상 커밋 집계를 읽어 결정론적인 리포트를 만듭니다. 결과에는 내용 실질·규모 범위를 반영한 점수, 협업 스타일, 실제 근거 경로, 수집 범위, 구조·위생 진단과 다음 도전이 포함됩니다.
...
- **공개된 기준:** [해석 가이드](https://myaiscore-production.up.railway.app/insights)에서 14개 파일 신호의 최대점과 스타일 판정 순서를 확인할 수 있습니다. 실제 획득점은 파일 존재·내용 실질·scanned-tree breadth를 함께 사용하며, 스타일은 총점 구간이 아니라 네 축의 분포로 정해집니다.
