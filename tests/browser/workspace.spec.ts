...
test('빈 내 리포트와 해석 가이드는 API를 호출하거나 결과를 꾸며내지 않는다', async ({ page }, testInfo) => {
  ...
  await expect(mixedProfileTone.nth(1)).toHaveClass(/repo-pole-violet/);
  await expect(mixedProfileTone.nth(2)).toHaveClass(/repo-pole-violet/);
  await expect(mixedProfileTone.nth(3)).toHaveClass(/repo-pole-violet/);
